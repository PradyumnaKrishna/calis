...
export function DashboardScreen({profileId}: DashboardScreenProps) {
  const api = useApi();
  ...
  const selectedWorkout = useMemo(
    () => plan?.workouts.find((workout) => workout.day === selectedDay),
    [plan, selectedDay],
  );
        ...
        <ScrollView
          ...
          <View className="mt-8">
            <View className="flex-row items-start justify-between gap-4">
              <View className="flex-1">
                <Text className="text-2xl font-black leading-8 text-foreground-light dark:text-foreground-dark">
                  {todaysWorkout ? todaysWorkout.title : 'Rest day'}
                </Text>
              </View>
              <View
                className={[
            ...
            {todaysWorkout ? (
              <Pressable
                ...
                onPress={() => {
                  setSelectedDay(todayAsPlanDay());
                }}>
                {todaysWorkout.exercises.map((exercise) => (
                  <View className="flex-row items-center gap-3" key={exercise.exerciseId}>
                    <View className="h-9 w-9 items-center justify-center rounded-full bg-surface-light dark:bg-surface-dark">
                      <NativeWindFeather
                        className="text-muted-light dark:text-muted-dark"
                    ...
                    <View className="flex-1">
                      ...
                      <Text className="mt-0.5 text-sm font-semibold text-muted-light dark:text-muted-dark">
          ...
          <View className="mt-8 border-t border-border-light pt-6 dark:border-border-dark">
            <View className="flex-row justify-between">
              {days.map((item) => {
                const workout = plan.workouts.find((candidate) => candidate.day === item.day);
